...
pub fn parser() -> impl Parser<char, Format, Error = Simple<char>> {
...
mod tests {
    ...
    #[test]
    fn ok_journal() {
        let result = parser().parse("journal");
        assert_eq!(result, Ok(Format::Journal));
    }

    #[test]
    fn ok_timeclock() {
        let result = parser().parse("timeclock");
        assert_eq!(result, Ok(Format::Timeclock));
    }

    #[test]
    fn ok_timedot() {
        let result = parser().parse("timedot");
    ...
    #[test]
    fn ok_rules() {
        let result = parser().parse("rules");
        assert_eq!(result, Ok(Format::Rules));
    }

    #[test]
    fn err() {
        let result = parser().parse("err");
